...
@section('content')
    <div class="mx-auto max-w-md rounded-lg border border-gray-700 bg-gray-800 p-8 shadow-lg">
        <h1 class="mb-2 text-2xl font-semibold text-white">Daftar Akun Baru</h1>
        <p class="mb-6 text-sm text-gray-400">Buat akun baru untuk mulai menggunakan platform</p>
        ...
        <form action="{{ route('register') }}" method="POST" class="space-y-5">
            ...
            <div>
                <label for="role" class="mb-2 block text-sm font-medium text-gray-300">Peran</label>
                <select
                    id="role"
                    name="role"
                    required
                    class="w-full rounded-lg border border-gray-600 bg-gray-700 px-4 py-2.5 text-sm text-white focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-colors"
                >
                    <option value="" disabled {{ old('role') ? '' : 'selected' }} class="bg-gray-700">Pilih peran</option>
                    <option value="mahasiswa" {{ old('role') === 'mahasiswa' ? 'selected' : '' }} class="bg-gray-700">Mahasiswa</option>
                    <option value="dosen" {{ old('role') === 'dosen' ? 'selected' : '' }} class="bg-gray-700">Dosen</option>
                </select>
                @error('role')
                    <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <button type="button" onclick="toggleRegisterPassword()"
                        ...
                        <svg id="register-eye-icon" class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                        </svg>
                        <svg id="register-eye-off-icon" class="h-5 w-5 hidden" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div>
                ...
                <div class="relative">
                    <input
                        ...
                        class="w-full rounded-lg border border-gray-600 bg-gray-700 px-4 py-2.5 pr-10 text-sm text-white placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-colors"
                    >
                    <button type="button" onclick="toggleRegisterPasswordConfirmation()"
                        class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-300 transition-colors"
                        id="register-password-confirmation-toggle">
                        <svg id="register-confirm-eye-icon" class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                        </svg>
                        <svg id="register-confirm-eye-off-icon" class="h-5 w-5 hidden" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
                        </svg>
            ...
            <p class="text-center text-sm text-gray-400">
                Sudah punya akun?
                <a href="{{ route('login') }}" class="text-blue-400 hover:text-blue-300 hover:underline transition-colors">Masuk di sini</a>
            </p>
        </form>
    ...
    <script>
        ...
        function toggleRegisterPasswordConfirmation() {
            const passwordInput = document.getElementById('password_confirmation');
            const eyeIcon = document.getElementById('register-confirm-eye-icon');
            const eyeOffIcon = document.getElementById('register-confirm-eye-off-icon');
