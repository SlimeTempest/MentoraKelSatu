...
@section('content')
    ...
    {{-- Error Alert --}}
    @if ($errors->has('job'))
        <div class="mb-6 rounded-xl border border-red-500/30 bg-gradient-to-r from-red-500/10 to-red-600/10 px-4 py-3 backdrop-blur-sm">
            ...
            <section class="rounded-2xl border border-gray-700/50 bg-gray-800/50 backdrop-blur-sm shadow-2xl">
                ...
                @else
                    <div class="overflow-hidden">
                                ...
                                <thead class="bg-gradient-to-r from-gray-700/50 to-gray-700/30">
                                    <tr>
                                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-300">
                                            Job Details</th>
                                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-300 hidden lg:table-cell">
                                            Creator</th>
                                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-300 hidden md:table-cell">
                                            Assignee</th>
                                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-300">
                                            Status</th>
                                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-300 hidden sm:table-cell">
                                            Price</th>
                                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-300 hidden lg:table-cell">
                                            Deadline</th>
                                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-300 hidden xl:table-cell">
                                            Categories</th>
                                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-300">
                                            Actions</th>
                                    </tr>
                                        ...
                                        <tr class="group transition-all duration-200 hover:bg-gray-700/30">
                                            <td class="px-6 py-4">
                                                <div class="space-y-1.5">
                                                    ...
                                                    <div class="flex flex-wrap items-center gap-2 sm:hidden">
                                                        <span class="inline-flex items-center gap-1 rounded-lg bg-green-500/20 px-2 py-0.5 text-xs font-semibold text-green-300">
                                                    ...
                                                    <div class="flex flex-wrap gap-1.5 xl:hidden">
                                                        @foreach ($job->categories->take(2) as $category)
                                                            <span class="inline-flex items-center rounded-full border border-blue-500/30 bg-blue-500/20 px-2 py-0.5 text-xs font-medium text-blue-300">
                                                                {{ $category->name }}
                                                            </span>
                                                        @endforeach
                                                        @if ($job->categories->count() > 2)
                                                            <span class="text-xs text-gray-400">+{{ $job->categories->count() - 2 }}</span>
                                                        @endif
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap hidden lg:table-cell">
                                                <a href="{{ route('users.profile.show', $job->creator) }}"
                                                    ...
                                                    <div class="h-8 w-8 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-xs font-bold text-white">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap hidden md:table-cell">
                                                @if ($job->assignee)
                                                    <a href="{{ route('users.profile.show', $job->assignee) }}"
                                                        class="inline-flex items-center gap-2 text-sm font-medium text-blue-400 transition-colors hover:text-blue-300">
                                                        <div class="h-8 w-8 rounded-full bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center text-xs font-bold text-white">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                @php
                                                    $statusConfig = [
                                                        'belum_diambil' => ['bg' => 'bg-gray-500/20', 'text' => 'text-gray-300', 'border' => 'border-gray-500/30', 'icon' => 'M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z'],
                                                        'on_progress' => ['bg' => 'bg-blue-500/20', 'text' => 'text-blue-300', 'border' => 'border-blue-500/30', 'icon' => 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z'],
                                                        'selesai' => ['bg' => 'bg-green-500/20', 'text' => 'text-green-300', 'border' => 'border-green-500/30', 'icon' => 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z'],
                                                        'kadaluarsa' => ['bg' => 'bg-red-500/20', 'text' => 'text-red-300', 'border' => 'border-red-500/30', 'icon' => 'M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z'],
                                                    ];
                                                    $status = $statusConfig[$job->status] ?? $statusConfig['belum_diambil'];
                                                @endphp
                                                <span class="inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-semibold capitalize {{ $status['bg'] }} {{ $status['text'] }} {{ $status['border'] }}">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap hidden sm:table-cell">
                                                <div class="inline-flex items-center gap-1.5 rounded-lg bg-green-500/20 px-3 py-1.5">
                                                    <svg class="h-4 w-4 text-green-400" fill="currentColor" viewBox="0 0 20 20">
                                                        <path d="M8.433 7.418c.155-.103.346-.196.567-.267v1.698a2.305 2.305 0 01-.567-.267C8.07 8.34 8 8.114 8 8c0-.114.07-.34.433-.582zM11 12.849v-1.698c.22.071.412.164.567.267.364.243.433.468.433.582 0 .114-.07.34-.433.582a2.305 2.305 0 01-.567.267z"></path>
                                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-13a1 1 0 10-2 0v.092a4.535 4.535 0 00-1.676.662C6.602 6.234 6 7.009 6 8c0 .99.602 1.765 1.324 2.246.48.32 1.054.545 1.676.662v1.941c-.391-.127-.68-.317-.843-.504a1 1 0 10-1.51 1.31c.562.649 1.413 1.076 2.353 1.253V15a1 1 0 102 0v-.092a4.535 4.535 0 001.676-.662C13.398 13.766 14 12.991 14 12c0-.99-.602-1.765-1.324-2.246A4.535 4.535 0 0011 9.092V7.151c.391.127.68.317.843.504a1 1 0 101.511-1.31c-.563-.649-1.413-1.076-2.354-1.253V5z" clip-rule="evenodd"></path>
                                                    </svg>
                                                    <span class="text-sm font-bold text-green-400">Rp {{ number_format($job->price, 0, ',', '.') }}</span>
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-6 py-4 hidden xl:table-cell">
                                                <div class="flex flex-wrap gap-1.5">
                                                    @foreach ($job->categories as $category)
                                                        <span class="inline-flex items-center rounded-full border border-indigo-500/30 bg-indigo-500/20 px-2.5 py-0.5 text-xs font-medium text-indigo-300">
                                                            {{ $category->name }}
                                                        </span>
                                                    @endforeach
                                                </div>
                        ...
                        <div class="border-t border-gray-700/50 bg-gradient-to-r from-gray-700/30 to-gray-700/20 px-6 py-4">
                            <div class="flex flex-col items-center justify-between gap-4 sm:flex-row">
                                <div class="text-sm text-gray-400">
                                    Menampilkan <span class="font-semibold text-gray-300">{{ $allJobs->firstItem() ?? 0 }}</span> sampai <span class="font-semibold text-gray-300">{{ $allJobs->lastItem() ?? 0 }}</span> dari <span class="font-semibold text-gray-300">{{ $allJobs->total() }}</span> hasil
                                </div>
                                <div>
                ...
                <div class="border-b border-gray-700/50 bg-gradient-to-r from-gray-700/50 to-gray-700/30 px-6 py-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400 mt-0.5">Job yang belum diambil. Kamu bisa ambil maksimal 2 sekaligus.</p>
                        </div>
                    </div>
                </div>
                    ...
                    @else
                        <div class="space-y-4">
                            @foreach ($availableJobs as $job)
                                <article class="group relative overflow-hidden rounded-xl border border-gray-700/50 bg-gradient-to-br from-gray-800/50 to-gray-800/30 p-6 transition-all duration-300 hover:border-green-500/50 hover:shadow-lg hover:shadow-green-500/10">
                                        ...
                                        <div class="flex-1 space-y-3">
                                            <div class="flex items-start justify-between gap-4">
                                                <div class="flex-1">
                                                    ...
                                                    <h3 class="text-lg font-bold text-white group-hover:text-green-400 transition-colors">{{ $job->title }}</h3>
                                                    <p class="mt-2 text-sm leading-relaxed text-gray-300 line-clamp-2">{{ Str::limit($job->description, 160) }}</p>
                                                </div>
                                            </div>
                                            <div class="flex flex-wrap items-center gap-2">
                                                <span class="inline-flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/30 px-3 py-1.5 text-xs font-bold text-green-300">
                                                ...
                                                @if ($job->deadline)
                                                    <span class="inline-flex items-center gap-1.5 rounded-lg bg-amber-500/20 border border-amber-500/30 px-3 py-1.5 text-xs font-medium text-amber-300">
                                                        <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                                        </svg>
                        ...
                        <div class="mt-6 flex flex-col items-center justify-between gap-4 border-t border-gray-700/50 pt-4 sm:flex-row">
                            <div class="text-sm text-gray-400">
                                Menampilkan <span class="font-semibold text-gray-300">{{ $availableJobs->firstItem() ?? 0 }}</span> sampai <span class="font-semibold text-gray-300">{{ $availableJobs->lastItem() ?? 0 }}</span> dari <span class="font-semibold text-gray-300">{{ $availableJobs->total() }}</span> hasil
                            </div>
                            <div>
                    ...
                    @else
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-700/30">
                                <thead class="bg-gradient-to-r from-gray-700/50 to-gray-700/30">
                                    <tr>
                                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-300">Job Details</th>
                                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-300 hidden sm:table-cell">Status</th>
                                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-300 hidden md:table-cell">Assignee</th>
                                        ...
                                        <tr class="group transition-all duration-200 hover:bg-gray-700/30">
                                            ...
                                            <td class="px-6 py-4 hidden sm:table-cell">
                                                @php
                                                    ...
                                                    $color = $statusColors[$job->status] ?? 'bg-gray-500/20 text-gray-300 border-gray-500/30';
                                                @endphp
                                                <span class="inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold capitalize {{ $color }}">{{ str_replace('_', ' ', $job->status) }}</span>
                                            </td>
                        ...
                        <div class="mt-6 flex flex-col items-center justify-between gap-4 border-t border-gray-700/50 bg-gradient-to-r from-gray-700/30 to-gray-700/20 px-6 py-4 sm:flex-row">
                            <div class="text-sm text-gray-400">
                                Menampilkan <span class="font-semibold text-gray-300">{{ $myJobs->firstItem() ?? 0 }}</span> sampai <span class="font-semibold text-gray-300">{{ $myJobs->lastItem() ?? 0 }}</span> dari <span class="font-semibold text-gray-300">{{ $myJobs->total() }}</span> hasil
                            </div>
                            <div>
                    ...
                    @else
                        <div class="space-y-4">
                            @foreach ($assignedJobs as $job)
                                <article class="group relative overflow-hidden rounded-xl border border-gray-700/50 bg-gradient-to-br from-gray-800/50 to-gray-800/30 p-6 transition-all duration-300 hover:border-indigo-500/50 hover:shadow-lg hover:shadow-indigo-500/10">
                                    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                                        <div class="flex-1 space-y-3">
                                            <div class="flex items-start justify-between gap-4">
                                                <div class="flex-1">
                                            ...
                                            <div class="flex flex-wrap items-center gap-2">
                                                @php
                                                    ...
                                                    $color = $statusColors[$job->status] ?? 'bg-gray-500/20 text-gray-300 border-gray-500/30';
                                                @endphp
                                                <span class="inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold capitalize {{ $color }}">
                                                    {{ str_replace('_', ' ', $job->status) }}
                                                </span>
                                                @if ($job->deadline)
                                                    <span class="inline-flex items-center gap-1.5 rounded-lg bg-amber-500/20 border border-amber-500/30 px-3 py-1.5 text-xs font-medium text-amber-300">
                                        ...
                                        <div class="flex flex-wrap gap-2 sm:flex-shrink-0">
                                            @if ($job->status === \App\Models\Job::STATUS_PROGRESS)
                                                <form action="{{ route('jobs.complete', $job) }}" method="POST" class="w-full sm:w-auto">
                                                    @csrf
                                                    <button type="submit"
                        ...
                        <div class="mt-6 flex flex-col items-center justify-between gap-4 border-t border-gray-700/50 pt-4 sm:flex-row">
                            <div class="text-sm text-gray-400">
                                Menampilkan <span class="font-semibold text-gray-300">{{ $assignedJobs->firstItem() ?? 0 }}</span> sampai <span class="font-semibold text-gray-300">{{ $assignedJobs->lastItem() ?? 0 }}</span> dari <span class="font-semibold text-gray-300">{{ $assignedJobs->total() }}</span> hasil
                            </div>
                            <div>
